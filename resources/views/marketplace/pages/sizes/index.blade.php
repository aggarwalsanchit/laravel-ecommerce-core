{{-- resources/views/marketplace/pages/sizes/index.blade.php --}}
@extends('management.layouts.app')

@section('title', 'Product Sizes')

@section('content')
<div class="page-content">
    <div class="page-container">
        <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
            <div class="flex-grow-1">
                <h4 class="fs-18 text-uppercase fw-bold mb-0">Product Sizes</h4>
                <p class="text-muted mb-0">Browse available sizes for your products</p>
            </div>
            <div class="text-end">
                <ol class="breadcrumb m-0 py-0">
                    <li class="breadcrumb-item"><a href="{{ route('vendor.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Sizes</li>
                </ol>
            </div>
        </div>

        {{-- Info Alert --}}
        <div class="alert alert-info mb-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    <i class="ti ti-info-circle me-2"></i>
                    These are the available sizes for your products.
                    @if($pendingRequestsCount > 0)
                        <strong>You have {{ $pendingRequestsCount }} pending size request(s).</strong>
                    @endif
                </div>
                <div>
                    <a href="{{ route('vendor.sizes.request.create') }}" class="btn btn-sm btn-warning">
                        <i class="ti ti-plus"></i> Request New Size
                    </a>
                    <a href="{{ route('vendor.sizes.requests.index') }}" class="btn btn-sm btn-secondary ms-2">
                        <i class="ti ti-list"></i> My Requests
                        @if($pendingRequestsCount > 0)
                            <span class="badge bg-light text-dark ms-1">{{ $pendingRequestsCount }}</span>
                        @endif
                    </a>
                </div>
            </div>
        </div>

        {{-- Search and Filter --}}
        <div class="card mb-4">
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4">
                        <div class="input-group">
                            <input type="text" class="form-control" id="searchInput" 
                                   placeholder="Search by size name or code..." 
                                   value="{{ request('search') }}">
                            <button class="btn btn-primary" id="searchBtn">
                                <i class="ti ti-search"></i>
                            </button>
                            <button class="btn btn-secondary" id="clearSearch" style="display: none;">
                                <i class="ti ti-x"></i> Clear
                            </button>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <select class="form-select" id="genderFilter">
                            <option value="">All Genders</option>
                            <option value="Men" {{ request('gender') == 'Men' ? 'selected' : '' }}>Men</option>
                            <option value="Women" {{ request('gender') == 'Women' ? 'selected' : '' }}>Women</option>
                            <option value="Unisex" {{ request('gender') == 'Unisex' ? 'selected' : '' }}>Unisex</option>
                            <option value="Kids" {{ request('gender') == 'Kids' ? 'selected' : '' }}>Kids</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select class="form-select" id="categoryFilter">
                            <option value="">All Categories</option>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ request('category_id') == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
                        <select class="form-select" id="sortFilter">
                            <option value="order" {{ request('sort_by') == 'order' ? 'selected' : '' }}>Default Order</option>
                            <option value="name" {{ request('sort_by') == 'name' ? 'selected' : '' }}>Name (A-Z)</option>
                            <option value="code" {{ request('sort_by') == 'code' ? 'selected' : '' }}>Code (A-Z)</option>
                            <option value="usage_count" {{ request('sort_by') == 'usage_count' ? 'selected' : '' }}>Most Used</option>
                        </select>
                    </div>
                </div>
            </div>
        </div>

        {{-- Sizes Grid --}}
        <div class="row">
            @forelse($sizes as $size)
                <div class="col-md-3 col-lg-2 mb-4">
                    <div class="card h-100 size-card text-center">
                        <div class="card-body">
                            <div class="size-icon mb-3">
                                <i class="ti ti-ruler fs-1 text-primary"></i>
                            </div>
                            <h6 class="card-title mb-1">{{ $size->name }}</h6>
                            <code class="text-muted small">{{ $size->code }}</code>
                            <div class="mt-2">
                                <span class="badge bg-{{ $size->gender == 'Men' ? 'primary' : ($size->gender == 'Women' ? 'danger' : ($size->gender == 'Unisex' ? 'info' : 'success')) }}">
                                    {{ $size->gender }}
                                </span>
                            </div>
                            <div class="mt-2">
                                @if($size->is_featured)
                                    <span class="badge bg-warning text-dark"><i class="ti ti-star"></i> Featured</span>
                                @endif
                                @if($size->is_popular)
                                    <span class="badge bg-danger"><i class="ti ti-fire"></i> Popular</span>
                                @endif
                            </div>
                            <div class="mt-2 small text-muted">
                                @if($size->us_size)
                                    US: {{ $size->us_size }}
                                @endif
                                @if($size->eu_size)
                                    | EU: {{ $size->eu_size }}
                                @endif
                            </div>
                        </div>
                        <div class="card-footer bg-transparent text-center">
                            <a href="{{ route('vendor.sizes.show', $size->id) }}" class="btn btn-sm btn-outline-primary">
                                <i class="ti ti-eye"></i> View Details
                            </a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-12">
                    <div class="text-center py-5">
                        <i class="ti ti-ruler-off" style="font-size: 64px; opacity: 0.5;"></i>
                        <h4 class="mt-3">No Sizes Found</h4>
                        <p class="text-muted">No sizes are available at the moment.</p>
                        <a href="{{ route('vendor.sizes.request.create') }}" class="btn btn-primary mt-2">
                            <i class="ti ti-plus"></i> Request a Size
                        </a>
                    </div>
                </div>
            @endforelse
        </div>

        {{-- Pagination --}}
        <div class="mt-4">
            {{ $sizes->appends(request()->query())->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
$(document).ready(function() {
    function applyFilters() {
        let search = $('#searchInput').val();
        let gender = $('#genderFilter').val();
        let categoryId = $('#categoryFilter').val();
        let sortBy = $('#sortFilter').val();
        
        let url = '{{ route("vendor.sizes.index") }}?';
        let params = [];
        
        if (search) params.push('search=' + encodeURIComponent(search));
        if (gender) params.push('gender=' + gender);
        if (categoryId) params.push('category_id=' + categoryId);
        if (sortBy && sortBy !== 'order') params.push('sort_by=' + sortBy);
        
        window.location.href = url + params.join('&');
    }

    $('#searchBtn').on('click', applyFilters);
    $('#genderFilter, #categoryFilter, #sortFilter').on('change', applyFilters);
    
    $('#searchInput').on('keypress', function(e) {
        if (e.which === 13) applyFilters();
    });

    $('#clearSearch').on('click', function() {
        $('#searchInput').val('');
        applyFilters();
    });

    if ($('#searchInput').val()) $('#clearSearch').show();
});
</script>
@endpush

@push('styles')
<style>
    .size-card {
        transition: transform 0.2s, box-shadow 0.2s;
        cursor: pointer;
    }
    .size-card:hover {
        transform: translateY(-5px);
        box-shadow: 0 10px 20px rgba(0,0,0,0.1);
    }
    .size-icon {
        transition: transform 0.2s;
    }
    .size-card:hover .size-icon {
        transform: scale(1.05);
    }
    .card-footer {
        border-top: none;
        padding-top: 0;
    }
</style>
@endpush