{{-- resources/views/marketplace/pages/colors/request-details.blade.php --}}
    ...
    <div class="page-container">
        <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
            ...
            <div class="text-end">
                <ol class="breadcrumb m-0 py-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('vendor.colors.requests.index') }}">My Requests</a></li>
        ...
        <div class="row">
            <div class="col-lg-8 mx-auto">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <div style="width: 60px; height: 60px; background-color: {{ $request->requested_code }}; border-radius: 50%; border: 2px solid #ddd;"></div>
                            <div>
                        ...
                        <table class="table table-borderless">
                            @if($request->requested_rgb)
                            <tr><th width="150">RGB Value:</th><td>{{ $request->requested_rgb }}</td></tr>
                            @endif
                            @if($request->requested_hsl)
                            <tr><th>HSL Value:</th><td>{{ $request->requested_hsl }}</td></tr>
                            @endif
                ...
                {{-- Created Color Card --}}
                @if($request->status === 'approved' && $request->createdColor)
                <div class="card mb-3 bg-success-subtle">
                    <div class="card-header bg-success text-white">
                        <h5 class="card-title mb-0">Color Created</h5>
                    </div>
                    <div class="card-body">
                        <p>Your requested color has been created!</p>
                        <div class="d-flex align-items-center gap-3">
                            <div style="width: 50px; height: 50px; background-color: {{ $request->createdColor->code }}; border-radius: 50%; border: 1px solid #ddd;"></div>
                            <div>
                                <strong>{{ $request->createdColor->name }}</strong>
                                <br>
                                <small class="text-muted">{{ $request->createdColor->code }}</small>
                            </div>
                        </div>
                        <div class="mt-3">
                            <a href="{{ route('vendor.products.create') }}?color={{ $request->createdColor->id }}" class="btn btn-sm btn-primary">
                                <i class="ti ti-package"></i> Add Product with this Color
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-footer d-flex justify-content-end gap-2">
                        <a href="{{ route('vendor.colors.requests.index') }}" class="btn btn-secondary">
...
function cancelRequest(requestId, requestName) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let form = $('#cancelForm');
            form.attr('action', '{{ url("vendor/colors/requests") }}/' + requestId);
            $.ajax({
