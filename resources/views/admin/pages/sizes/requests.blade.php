{{-- resources/views/admin/pages/sizes/requests.blade.php --}}
@extends('management.layouts.app')

@section('title', 'Size Requests')
...
@section('content')
        ...
        <div class="page-container">
            <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
                <div class="flex-grow-1">
                    <h4 class="fs-18 text-uppercase fw-bold mb-0">Size Requests from Vendors</h4>
                </div>
                <div class="text-end">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <h3 class="card-title mb-0">Size Requests</h3>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="searchInput" placeholder="Search by size name or code..." value="{{ request('search') }}">
                                    ...
                                    <div class="d-flex gap-2 justify-content-end flex-wrap">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu" id="genderFilter">
                                                ...
                                                <li><a class="dropdown-item" href="#" data-gender="Men">Men</a></li>
                                                <li><a class="dropdown-item" href="#" data-gender="Women">Women</a></li>
                                                <li><a class="dropdown-item" href="#" data-gender="Unisex">Unisex</a></li>
                                                <li><a class="dropdown-item" href="#" data-gender="Kids">Kids</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <tbody>
                                        @forelse($requests as $request)
                                            @php
                                                ...
                                                $genderColor = $genderColors[$request->requested_gender] ?? 'secondary';
                                            @endphp
                                            <tr>
                                                <td><input type="checkbox" class="form-check-input request-checkbox" value="{{ $request->id }}"></td>
                                                <td>#{{ $request->id }}</td>
                                                <td>
                                                ...
                                                <td><code>{{ $request->requested_code }}</code></td>
                                                <td>
                                                    <span class="badge bg-{{ $genderColor }}">{{ $request->requested_gender }}</span>
                                                </td>
                                                <td>
                                                    @if($request->vendor)
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="{{ route('admin.sizes.requests.show', $request->id) }}" class="btn btn-info" title="View Details">
                                                            <i class="ti ti-eye"></i>
                                                        </a>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="9" class="text-center py-5">
                                                    ...
                                                    <p class="text-muted">When vendors request new sizes, they will appear here.</p>
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
    ...
    {{-- Bulk Action Form --}}
    <form id="bulkRequestForm" method="POST" action="{{ route('admin.sizes.requests.bulk-action') }}" style="display: none;">
        @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#searchBtn').on('click', function() {
                ...
                let status = '{{ $status }}';
                let gender = $('#genderFilter .dropdown-item.active').data('gender') || '';
                window.location.href = '{{ route("admin.sizes.requests") }}?search=' + search + '&status=' + status + '&gender=' + gender;
            });
            ...
            $('#clearSearch').on('click', function() {
                let status = '{{ $status }}';
                window.location.href = '{{ route("admin.sizes.requests") }}?status=' + status;
            });
            ...
            $('#statusFilter .dropdown-item').on('click', function(e) {
                ...
                let search = $('#searchInput').val();
                let gender = $('#genderFilter .dropdown-item.active').data('gender') || '';
                window.location.href = '{{ route("admin.sizes.requests") }}?status=' + status + '&search=' + search + '&gender=' + gender;
            });
            ...
            $('#genderFilter .dropdown-item').on('click', function(e) {
                ...
                let status = '{{ $status }}';
                let search = $('#searchInput').val();
                $('#genderFilter .dropdown-item').removeClass('active');
                $(this).addClass('active');
                window.location.href = '{{ route("admin.sizes.requests") }}?gender=' + gender + '&status=' + status + '&search=' + search;
            });
        ...
        function deleteRequest(requestId, requestName) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("admin/sizes/requests") }}/' + requestId,
                        type: 'DELETE',
                        data: { _token: '{{ csrf_token() }}' },
                        success: function(response) {
                            if (response.success) {
