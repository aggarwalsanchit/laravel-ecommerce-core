{{-- resources/views/admin/pages/colors/requests.blade.php --}}
@extends('management.layouts.app')

@section('title', 'Color Requests')
...
@section('content')
        ...
        <div class="page-container">
            <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
                <div class="flex-grow-1">
                    <h4 class="fs-18 text-uppercase fw-bold mb-0">Color Requests from Vendors</h4>
                </div>
                <div class="text-end">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <h3 class="card-title mb-0">Color Requests</h3>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center gap-2">
                                                        <div
                                                            style="width: 30px; height: 30px; background-color: {{ $request->requested_code }}; border-radius: 50%; border: 1px solid #ddd;">
                                                        </div>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="{{ route('admin.colors.requests.show', $request->id) }}"
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="8" class="text-center py-5">
                                                    ...
                                                    <h5 class="mt-3">No Color Requests Found</h5>
                                                    <p class="text-muted">When vendors request new colors, they will appear
                                                        here.</p>
                                                </td>
                                            </tr>
                                        @endforelse
    ...
    {{-- Bulk Action Form --}}
    <form id="bulkRequestForm" method="POST" action="{{ route('admin.colors.requests.bulk-action') }}"
        style="display: none;">
        @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#searchBtn').on('click', function() {
                ...
                let status = '{{ $status }}';
                window.location.href = '{{ route('admin.colors.requests') }}?search=' + search +
                    '&status=' + status;
            });
            ...
            $('#clearSearch').on('click', function() {
                window.location.href = '{{ route('admin.colors.requests') }}?status={{ $status }}';
            });
        ...
        function deleteRequest(requestId, requestName) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url('admin/colors/requests') }}/' + requestId,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
