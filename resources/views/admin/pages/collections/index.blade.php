...
@section('content')
        ...
        <div class="page-container">
            <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
                <div class="flex-grow-1">
                    <h4 class="fs-18 text-uppercase fw-bold mb-0">Collection Management</h4>
                </div>
                <div class="text-end">
                    ...
                    <div class="card bg-primary text-white">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <i class="ti ti-category" style="font-size: 40px; opacity: 0.5;"></i>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card bg-success text-white">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="mb-0">Active Collections</h6>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <h3 class="card-title mb-0">Collection Management</h3>
                            <div class="d-flex gap-2">
                                <a href="{{ route('admin.collections.analytics') }}" class="btn btn-info">
                        ...
                        <div class="card-body">
                            ...
                            <div id="collectionsTableContainer">
                                @include('admin.pages.collections.partials.collections-table', [
                            ...
                            <div id="paginationContainer" class="mt-3">
                                {{ $collections->appends(request()->query())->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    {{-- Bulk Action Form --}}
    <form id="bulkActionForm" method="POST" action="{{ route('admin.collections.bulk-action') }}"
        style="display: none;">
        @csrf
        <input type="hidden" name="action" id="bulkAction">
        <input type="hidden" name="collection_ids" id="bulkCollectionIds">
    </form>
@endsection
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadCollections() {
                $.ajax({
                    ...
                    data: currentFilters,
                    beforeSend: function() {
                        $('#collectionsTableContainer').html(
                            '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"></div></div>'
                        );
                        $('#paginationContainer').html('');
                    },
                    success: function(response) {
                        $('#collectionsTableContainer').html(response.table);
                        ...
                        $('#selectAll').off('change').on('change', function() {
                            $('.collection-checkbox').prop('checked', $(this).prop('checked'));
                        });

                        $('.collection-checkbox').off('change').on('change', function() {
                            let allChecked = $('.collection-checkbox:checked').length === $(
                                '.collection-checkbox').length;
                        ...
                        $('.toggle-status').off('change').on('change', function() {
                            ...
                            toggleStatus(collectionId, this);
                        });

                        // Reinitialize featured toggle switches
                        $('.toggle-featured').off('change').on('change', function() {
                            let collectionId = $(this).data('id');
                            toggleFeatured(collectionId, this);
                        });
                    },
                    error: function() {
                        $('#collectionsTableContainer').html(
                            '<div class="alert alert-danger">Error loading collections</div>');
                    }
                });
            }
        ...
        // Toggle Status
        function toggleStatus(collectionId, element) {
            let isChecked = $(element).prop('checked');

            $.ajax({
                url: '{{ url('admin/collections') }}/' + collectionId + '/toggle-status',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        // Toggle Featured
        function toggleFeatured(collectionId, element) {
            let isChecked = $(element).prop('checked');

            $.ajax({
                url: '{{ url('admin/collections') }}/' + collectionId + '/toggle-featured',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        function bulkAction(action) {
            ...
            if (selectedCollections.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'No Selection',
                    text: 'Please select at least one collection.',
            ...
            Swal.fire({
                title: `${actionText.toUpperCase()} Collections?`,
                text: `Are you sure you want to ${actionText} ${selectedCollections.length} selected collection(s)?`,
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $('#bulkCollectionIds').val(JSON.stringify(selectedCollections));
