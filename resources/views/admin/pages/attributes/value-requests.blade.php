{{-- resources/views/admin/pages/attributes/value-requests.blade.php --}}
@extends('management.layouts.app')

@section('title', 'Attribute Value Requests')
    ...
    <div class="page-container">
        <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
            <div class="flex-grow-1">
                <h4 class="fs-18 text-uppercase fw-bold mb-0">Attribute Value Requests from Vendors</h4>
            </div>
            <div class="text-end">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <h3 class="card-title mb-0">Attribute Value Requests</h3>
                        <div class="d-flex gap-2">
                            ...
                            <div class="btn-group">
                                <button type="button" class="btn btn-outline-secondary dropdown-toggle" data-bs-toggle="dropdown">
                                    <i class="ti ti-filter me-1"></i> Attribute
                                </button>
                                <ul class="dropdown-menu" id="attributeFilter">
                                    <li><a class="dropdown-item" href="#" data-attribute="">All Attributes</a></li>
                                    @foreach($attributes as $attribute)
                                        <li><a class="dropdown-item" href="#" data-attribute="{{ $attribute->id }}">{{ $attribute->name }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                            <a href="{{ route('admin.attributes.requests') }}" class="btn btn-secondary">
                                <i class="ti ti-list"></i> Attribute Requests
                            </a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <div class="input-group">
                                    <input type="text" class="form-control" id="searchInput" placeholder="Search by value name..." value="{{ request('search') }}">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        @if($request->reason)
                                                            <br><small class="text-muted">{{ Str::limit($request->reason, 50) }}</small>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <a href="{{ route('admin.attributes.value-requests.show', $request->id) }}" class="btn btn-info" title="View Details">
                                                        <i class="ti ti-eye"></i>
                                                    </a>
                                                    @if($request->status === 'pending')
                                                        @can('approve_attributes')
                                                            <button type="button" class="btn btn-success" onclick="approveValueRequest({{ $request->id }})" title="Approve">
                                                                <i class="ti ti-check"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-warning" onclick="showRejectModal({{ $request->id }}, '{{ $request->requested_value }}')" title="Reject">
                                                                <i class="ti ti-x"></i>
                                                            </button>
                                                        @endcan
                                                    @endif
                                                    @if($request->status !== 'approved')
                                                        @can('delete_attributes')
                                                            <button type="button" class="btn btn-danger" onclick="deleteValueRequest({{ $request->id }}, '{{ $request->requested_value }}')" title="Delete">
                                        ...
                                        <tr>
                                            <td colspan="10" class="text-center py-5">
                                                <div class="empty-state">
                                                    ...
                                                    <h5 class="mt-3">No Attribute Value Requests Found</h5>
                                                    <p class="text-muted">When vendors request new attribute values, they will appear here.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
...
{{-- Bulk Action Form --}}
<form id="bulkRequestForm" method="POST" action="{{ route('admin.attributes.value-requests.bulk-action') }}" style="display: none;">
    @csrf
...
$(document).ready(function() {
    ...
    $('#searchBtn').on('click', function() {
        ...
        let status = '{{ $status }}';
        let attributeId = $('#attributeFilter .dropdown-item.active').data('attribute') || '';
        window.location.href = '{{ route("admin.attributes.value-requests") }}?search=' + search + '&status=' + status + '&attribute_id=' + attributeId;
    });
    ...
    $('#clearSearch').on('click', function() {
        let status = '{{ $status }}';
        window.location.href = '{{ route("admin.attributes.value-requests") }}?status=' + status;
    });
    ...
    $('#statusFilter .dropdown-item').on('click', function(e) {
        ...
        let search = $('#searchInput').val();
        let attributeId = $('#attributeFilter .dropdown-item.active').data('attribute') || '';
        window.location.href = '{{ route("admin.attributes.value-requests") }}?status=' + status + '&search=' + search + '&attribute_id=' + attributeId;
    });

    // Attribute filter
    $('#attributeFilter .dropdown-item').on('click', function(e) {
        e.preventDefault();
        let attributeId = $(this).data('attribute');
        let status = '{{ $status }}';
        let search = $('#searchInput').val();
        $('#attributeFilter .dropdown-item').removeClass('active');
        $(this).addClass('active');
        window.location.href = '{{ route("admin.attributes.value-requests") }}?attribute_id=' + attributeId + '&status=' + status + '&search=' + search;
    });
...
function deleteValueRequest(requestId, requestValue) {
    Swal.fire({
        title: 'Delete Request?',
        text: `Are you sure you want to delete the request for "${requestValue}"?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#6c757d',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ url("admin/attributes/value-requests") }}/' + requestId,
                type: 'DELETE',
                data: { _token: '{{ csrf_token() }}' },
                success: function(response) {
                    if (response.success) {
