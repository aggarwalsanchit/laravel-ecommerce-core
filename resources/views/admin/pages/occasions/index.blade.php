{{-- resources/views/admin/occasions/index.blade.php --}}
...
@section('content')
        ...
        <div class="page-container">
            <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
                <div class="flex-grow-1">
                    <h4 class="fs-18 text-uppercase fw-bold mb-0">Occasion Management</h4>
                </div>
                <div class="text-end">
                    ...
                    <div class="card bg-primary text-white">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="mb-0">Total Occasions</h6>
                                    <h2 class="mb-0">{{ $statistics['total'] ?? 0 }}</h2>
                                </div>
                                <i class="ti ti-calendar-event" style="font-size: 40px; opacity: 0.5;"></i>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card bg-success text-white">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="mb-0">Active Occasions</h6>
                ...
                <div class="col-md-3">
                    <div class="card bg-warning text-dark">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="mb-0">Featured Occasions</h6>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <h3 class="card-title mb-0">Occasion Management</h3>
                            <div class="d-flex gap-2">
                                <a href="{{ route('admin.occasions.analytics') }}" class="btn btn-info">
                                    <i class="ti ti-chart-bar me-1"></i> Analytics
                                </a>
                                @can('create occasions')
                                    <a href="{{ route('admin.occasions.create') }}" class="btn btn-primary">
                        ...
                        <div class="card-body">
                            ...
                            {{-- Bulk Actions --}}
                            @canany(['edit occasions', 'delete occasions'])
                                <div class="row mb-3">
                                    <div class="col-12">
                            ...
                            {{-- Occasions Table Container --}}
                            <div id="occasionsTableContainer">
                                @include('admin.pages.occasions.partials.occasions-table', [
                                    'occasions' => $occasions,
                                ])
                            </div>
    ...
    {{-- Bulk Action Form --}}
    <form id="bulkActionForm" method="POST" action="{{ route('admin.occasions.bulk-action') }}" style="display: none;">
        @csrf
        <input type="hidden" name="action" id="bulkAction">
        <input type="hidden" name="occasion_ids" id="bulkOccasionIds">
    </form>
@endsection
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadOccasions() {
                $.ajax({
                    ...
                    data: currentFilters,
                    beforeSend: function() {
                        $('#occasionsTableContainer').html(
                            '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"></div></div>'
                            );
                        $('#paginationContainer').html('');
                    },
                    success: function(response) {
                        $('#occasionsTableContainer').html(response.table);
                        ...
                        $('#selectAll').off('change').on('change', function() {
                            $('.occasion-checkbox').prop('checked', $(this).prop('checked'));
                        });

                        $('.occasion-checkbox').off('change').on('change', function() {
                            let allChecked = $('.occasion-checkbox:checked').length === $(
                                '.occasion-checkbox').length;
                            $('#selectAll').prop('checked', allChecked);
                        });

                        // Reinitialize status toggle switches
                        $('.toggle-status').off('change').on('change', function() {
                            let occasionId = $(this).data('id');
                            toggleStatus(occasionId, this);
                        });

                        // Reinitialize featured toggle switches
                        $('.toggle-featured').off('change').on('change', function() {
                            let occasionId = $(this).data('id');
                            toggleFeatured(occasionId, this);
                        });
                    },
                    error: function() {
                        $('#occasionsTableContainer').html(
                            '<div class="alert alert-danger">Error loading occasions</div>');
                    }
                });
            }
        ...
        // Toggle Status
        function toggleStatus(occasionId, element) {
            let isChecked = $(element).prop('checked');

            $.ajax({
                url: '{{ url('admin/occasions') }}/' + occasionId + '/toggle-status',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        // Toggle Featured
        function toggleFeatured(occasionId, element) {
            let isChecked = $(element).prop('checked');

            $.ajax({
                url: '{{ url('admin/occasions') }}/' + occasionId + '/toggle-featured',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        function bulkAction(action) {
            ...
            Swal.fire({
                title: `${actionText.toUpperCase()} Occasions?`,
                text: `Are you sure you want to ${actionText} ${selectedOccasions.length} selected occasion(s)?`,
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $('#bulkOccasionIds').val(JSON.stringify(selectedOccasions));
