            ...
            <tr>
                ...
                <td>
                    <div>
                        ...
                        @if ($user->phone)
                            <small><i class="ti ti-phone me-1"></i>
                                {{ $user->phone }}</small>
                        @endif
                    </div>
                </td>
                <td>
                    ...
                    @empty
                        <span class="badge bg-secondary-subtle text-secondary p-2">No
                            Role</span>
                    @endforelse
                </td>
                <td>
                    ...
                    <div class="hstack gap-1 justify-content-end">
                        ...
                        @can('delete users')
                            <button type="button" class="btn btn-soft-danger btn-icon btn-sm rounded-circle"
                                onclick="confirmDelete({{ $user->id }})" data-bs-toggle="tooltip" title="Delete User"
                        ...
                        @can('activate users')
                            @if (!$user->is_active)
                                <button type="button" class="btn btn-soft-warning btn-icon btn-sm rounded-circle"
                                    onclick="confirmActivate({{ $user->id }})" data-bs-toggle="tooltip"
                                    title="Activate User">
                        ...
                        @can('impersonate users')
                            <button type="button" class="btn btn-soft-info btn-icon btn-sm rounded-circle"
                                onclick="impersonateUser({{ $user->id }})" data-bs-toggle="tooltip"
